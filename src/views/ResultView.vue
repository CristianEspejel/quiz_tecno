...
<template>
  <div class="result-container max-w-4xl mx-auto p-4 sm:p-6 md:p-8">
    ...
    <div class="questions-detail">
      ...
      <div
        v-for="(question, qIndex) in allQuestions"
        :key="qIndex"
        class="question-card mb-6 p-4 bg-white rounded-lg shadow-sm border border-gray-100"
      >
        <!-- Pregunta con soporte de bloques ```code``` -->
        <p class="font-medium mb-3 text-gray-800 whitespace-pre-wrap" v-html="renderQuestion(question.pregunta)">
        </p>

        <!-- Opciones (numeradas) -->
        <div class="options space-y-2">
          <div
            v-for="(option, oIndex) in question.opciones"
            :key="oIndex"
            :class="[
              'option p-3 rounded-md border',
              option.clave === question.respuesta_correcta ? 'bg-green-50 border-green-200' : 'border-gray-200',
              userAnswers[qIndex] === option.clave && userAnswers[qIndex] !== question.respuesta_correcta
                ? 'bg-red-50 border-red-200' : ''
            ]"
          >
            <span class="font-medium">{{ oIndex + 1 }})</span>
            <span class="ml-2 whitespace-pre-wrap">{{ option.texto }}</span>

            <span v-if="option.clave === question.respuesta_correcta" class="ml-2 text-green-600">
              ✓ Correcta
            </span>
            <span
              v-if="userAnswers[qIndex] === option.clave && userAnswers[qIndex] !== question.respuesta_correcta"
              class="ml-2 text-red-600"
            >
              ✗ Tu elección
            </span>
        ...
        <!-- Explicación -->
        <div v-if="question.explicacion" class="mt-4 text-gray-800">
          <span class="font-semibold">Explicación:</span>
          <span class="ml-1">{{ question.explicacion }}</span>
        </div>

        <!-- Recursos -->
        <div v-if="question.recursos && question.recursos.length" class="mt-2 text-gray-800">
          <span class="font-semibold">Recursos:</span>
          <ul class="list-disc ml-6 mt-1">
            <li v-for="(r, idx) in question.recursos" :key="idx">
              <a
                :href="r.url"
...
const effectiveness = computed(() => {
  ...
  return ((score.value / t) * 100).toFixed(2)
})

// Banco y respuestas del usuario
const allQuestions = ref([])
const userAnswers = ref([])
...
onMounted(() => {
  ...
  // datos del localStorage (guardados en QuizView)
  const quizData = JSON.parse(localStorage.getItem('quizData') || '{}')
  allQuestions.value = quizData.questions || []
  userAnswers.value = quizData.userAnswers || []
})

// Render básico de bloques ```code``` dentro de la pregunta
